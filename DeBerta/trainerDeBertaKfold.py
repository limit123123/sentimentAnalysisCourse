...
class config():
    ...
    def __init__(self):
        ...
        self.KFold = 5              # 交叉y
        self.output_path = ['./outputsKFold/{}Fold'.format(i) for i in range(self.KFold)]


def mergeData(path1,path2):
    with open(path1, 'r', encoding='UTF-8') as f:
        dataPath = json.load(f)
    with open(path2, 'r', encoding='UTF-8') as f:
        dataPath.extend(json.load(f))
    return dataPath



class myDataset(Dataset):
    def __init__(self, dataAll, tokenizer, with_label=True):
    ...
    def __getitem__(self, item):
        inputs = self.tokenizer(self.contents[item][0], max_length=300, padding='max_length', truncation=True)
        if self.with_label:
            return {
                **inputs,
...
if __name__ == '__main__':
    ...
    Data = mergeData(configModel.train_path, configModel.eval_path)
    kfold = KFold(n_splits=configModel.KFold, shuffle=True, random_state=0)
    for index, (trainIndex, evalIndex) in enumerate(kfold.split(Data)):
        trainData = [Data[i] for i in trainIndex]
        evalData = [Data[i] for i in evalIndex]
        model = AutoModelForSequenceClassification.from_pretrained(configModel.model_path, num_labels=3)
        tokenizer = BertTokenizer.from_pretrained(configModel.tokenizer_path)
        trainDataset = myDataset(trainData, tokenizer, with_label=True)
        evalDataset = myDataset(evalData, tokenizer, with_label=True)
        train_args = TrainingArguments(
            output_dir=configModel.output_path[index],
